#include "rbis_realsense_update.hpp"
#include <path_util/path_util.h>

namespace MavStateEst {

RealsenseHandler::RealsenseHandler(lcm::LCM* lcm_recv,  lcm::LCM* lcm_pub,
  ...
  char* mode_str = bot_param_get_str_or_fail(param, "state_estimator.realsense.mode");

  if (strcmp(mode_str, "velocity_rotation_rate") == 0) {
    mode = RealsenseHandler::MODE_VELOCITY_ROTATION_RATE;
    std::cout << "REALSENSE will provide velocity and rotation rates." << std::endl;
  }
  else if (strcmp(mode_str, "velocity") == 0){
    mode = RealsenseHandler::MODE_VELOCITY;
    std::cout << "REALSENSE will provide velocity rates." << std::endl;
  }
  else if (strcmp(mode_str, "position") == 0){
    mode = RealsenseHandler::MODE_POSITION;
    std::cout << "REALSENSE will provide position corrections." << std::endl;
  }
  else if (strcmp(mode_str, "position_orient") == 0){
    mode = RealsenseHandler::MODE_POSITION_ORIENT;
    std::cout << "REALSENSE will provide position and orientation corrections." << std::endl;
  }
  else{
    std::cout << "REALSENSE mode not understood "<< mode_str << ", exiting\n";
    exit(-1);
    // ... incomplete...
  }
  free(mode_str);
  
  Eigen::VectorXd R_realsense;
  if (mode == MODE_VELOCITY_ROTATION_RATE) {
    z_indices.resize(6);
    R_realsense.resize(6);
  }
  else if (mode == MODE_VELOCITY){
    z_indices.resize(3);
    R_realsense.resize(3);
  }
  ...
  if (mode == MODE_VELOCITY_ROTATION_RATE) {
    double R_realsense_vxyz = bot_param_get_double_or_fail(param, "state_estimator.realsense.r_vxyz");
    R_realsense(0) = bot_sq(R_realsense_vxyz);
    R_realsense(1) = bot_sq(R_realsense_vxyz);
    R_realsense(2) = bot_sq(R_realsense_vxyz);
    z_indices.head<3>() = eigen_utils::RigidBodyState::velocityInds();

    double R_realsense_vang = bot_param_get_double_or_fail(param, "state_estimator.realsense.r_vang");
    R_realsense(3) = bot_sq(R_realsense_vang);
    R_realsense(4) = bot_sq(R_realsense_vang);
    R_realsense(5) = bot_sq(R_realsense_vang);
    ...
    double R_realsense_pxyz = bot_param_get_double_or_fail(param, "state_estimator.realsense.r_pxyz");
    R_realsense(0) = bot_sq(R_realsense_pxyz);
    R_realsense(1) = bot_sq(R_realsense_pxyz);
    R_realsense(2) = bot_sq(R_realsense_pxyz);
    ...
    double R_realsense_chi = bot_param_get_double_or_fail(param, "state_estimator.realsense.r_chi");
    R_realsense(3) = bot_sq(R_realsense_chi);
    R_realsense(4) = bot_sq(R_realsense_chi);
    R_realsense(5) = bot_sq(R_realsense_chi);
  ...
  cov_realsense = R_realsense.asDiagonal();

  prev_t0_body_ = Eigen::Isometry3d::Identity();
  prev_t0_body_internal_ = Eigen::Isometry3d::Identity();
  prev_t0_body_utime_ = 0;

  publish_diagnostics_ = bot_param_get_boolean_or_fail(param, "state_estimator.realsense.publish_diagnostics");
...
RBISUpdateInterface * RealsenseHandler::processMessage(const pronto::update_t * msg, MavStateEstimator* state_estimator){

  if (msg->estimate_status == pronto::update_t::ESTIMATE_VALID){
  }else{
    std::cout << "RealsenseHandler: REALSENSE failure, not integrating this measurement\n";
    return NULL;
  }
  
  // TODO: explore why this is allowed to be published upstream
  if ( isnan( msg->translation[0]) ){
    std::cout << "RealsenseHandler: REALSENSE produced NaN. x="<< msg->translation[0] << ", quitting\n";
    exit(-1); // keep this exit until we have found the source of the NaN in Realsense
    return NULL;
  }
  ...
  if ( (mode == MODE_VELOCITY_ROTATION_RATE) || (mode == MODE_VELOCITY) ) {
    ...
    if (publish_diagnostics_){
      ...
      lcm_pub->publish("POSE_BODY_REALSENSE_VELOCITY", &vel_pose );      
    }    
  }
  ...
  if (mode == MODE_VELOCITY_ROTATION_RATE) {
    ...
    //return new RBISIndexedPlusOrientationMeasurement(z_indices, z_meas, cov_realsense, quat, RBISUpdateInterface::realsense,
    //        msg->timestamp);
    std::cout << "RealsenseHandler Mode not supported, exiting\n";
    return NULL;

  }else if (mode == MODE_VELOCITY) {
    return new RBISIndexedMeasurement(eigen_utils::RigidBodyState::velocityInds(),
        Eigen::Map<const Eigen::Vector3d>( odo_velT.trans_vec ), cov_realsense, RBISUpdateInterface::realsense,
        msg->timestamp);
    ...
    return new RBISIndexedMeasurement(eigen_utils::RigidBodyState::positionInds(),
        z_meas, cov_realsense, RBISUpdateInterface::realsense,
        msg->timestamp);
    ...
    return new RBISIndexedPlusOrientationMeasurement(z_indices,
         z_meas, cov_realsense, quat, RBISUpdateInterface::realsense,
         msg->timestamp);
...
BotTrans RealsenseHandler::getTransAsVelocityTrans(BotTrans msgT, int64_t utime, int64_t prev_utime){
...
void RealsenseHandler::sendTransAsVelocityPose(BotTrans msgT, int64_t utime, int64_t prev_utime, std::string channel){
